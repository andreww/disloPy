...
def readCij(basename,path='./'):
    '''Reads the elastic constants matrix Cij (in Gpa)
...
# This works -> need to make a useful function, though
def anisoDislocation(x,b,dummy1=0,dummy2=0):
    u = 0j*np.zeros(3)
    if abs(x[0]) > 1e-8 or abs(x[1]) > 1e-8:
        for i in range(3):
            x1 = np.dot(L[i],b)*A[i] \
                 * cmath.log(x[0]+p[i]*x[1])
            x2 = np.dot(L[i+3],b)*A[i+3] \
                 * cmath.log(x[0]+p[i+3]*x[1])
            u+=(x1-x2).copy()
        u = (1/(2*pi*1j)*u)
    return u.real
...
def makeAnisoField(Cij):
    ...
    def uAniso(x,b,dummy1=0,dummy2=0):
