...
def R(atom):
    ...
    # use last two elements of coordinates as dislocation line lies
    # along <100> in Gulp
    distance = L.norm(atom.getCoordinates()[1:])
    return distance
...
def newRegions(rIPerfect,rIDislocated,rIIPerfect,rIIDislocated,Rnew):
    ...
    for i,atom in enumerate(rIDislocated.getAtoms()):
        ...
        if Rxy < Rnew:
            ...
            newRI.addAtom(atom)
            newRIPerfect.addAtom(rIPerfect.getAtoms()[i])
        else:
            # fix atom in region II if Rxy greater than new radius
            newRII.addAtom(atom)
            newRIIPerfect.addAtom(rIPerfect.getAtoms()[i])
...
def writeSinglePoint(rIBasis,rIIBasis,sysInfo,outName,disloc):
    ...
    if rIBasis.getAtoms():    
        gulp.writeRegion(rIBasis,dummyLattice,outStream,1,disloc,toCart=False,
                                                       coordType='pfractional')
    if rIIBasis.getAtoms():
        gulp.writeRegion(rIIBasis,dummyLattice,outStream,2,disloc,toCart=False,
                                                        coordType='pfractional')
...
def iterateOverRI(startRI,finalRI,dRI,baseName,gulpExec,
    ...
    dislocationFile = gulp.readGulpFile('dis.' + baseName + '.grs')
    perfectFile = gulp.readGulpFile('ndf.' + baseName + '.grs')
    ...
    gulp.extractRegions(perfectFile,rIPerfect,rIIPerfect)
    gulp.extractRegions(dislocationFile,rIDislocated,rIIDislocated)
    ...
    while currentRI >= finalRI:
        ...
        if explicitRegions: # use GULP's eregion functionality     
            ...
            for cellType in ('dislocated','perfect'):                                             
                os.system('%s < %s.%s.gin > %s.%s.gout' %  (gulpExec,derivedName,
                                                 cellType, derivedName,cellType))
            ...
            for cellType in ('dislocated','perfect'):
                for regionsUsed in ('RI','RII','BOTH'):
                    os.system('%s < %s.%s.%s.gin > %s.%s.%s.gout' %  (gulpExec,
                                   derivedName,cellType,regionsUsed,derivedName,
                                                          cellType,regionsUsed))
...
def EDis(r,Ecore,K,b,rcore=10.):
    ...
    return Ecore + K*b**3/(4*np.pi)*np.log(r/rcore)
    
def fitCoreEnergy(basename,b,rcore=10):
    ...
    # define dislocation energy function -> contains
    def specificEnergy(r,Ecore,K):
        return Ecore +K*b**3/(4*np.pi)*np.log(r/rcore)
        
    # fit the core energy and energy coefficient
    par,cov = curve_fit(specificEnergy,E[:,0],E[:,1])
    Ecore = par[0]; K = par[1]
...
def main(argv):
    ...
    Ecore,K,cov = fitCoreEnergy(basename,b,2*b)
    KGPa = K*CONV_EV_TO_GPA 
    errKGPa = np.sqrt(cov[1,1])*CONV_EV_TO_GPA
    EString = "Core energy: %.4f +- %.4f eV/angstrom" % (Ecore/b,np.sqrt(cov[0,0])/b)
