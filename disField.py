...
def plotDisplacementFieldXY(fieldType,figureName,Sij=0.3):
    ...
    for i in range(gridSize):
        ...
        for j in range(gridSize):
            ...
            u = fieldType(np.array([x[i,j],y[i,j]]),np.array([0.,0.,1.]),
                                                    np.array([0.,0.]),Sij)
            tempx.append(u[0])
            tempy.append(u[1])
...
def plotDisplacementFieldZ(fieldType,figureName,Sij=0.3):
    ...
    for i in range(gridSize):
        ...
        for j in range(gridSize):
            # calculate displacement field and extract x and y components
            u = fieldType(np.array([x[i,j],y[i,j]]),1.,np.array([0.,0.]),Sij)
            temp.append(u[2])
